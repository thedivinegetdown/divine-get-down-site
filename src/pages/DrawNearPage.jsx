...
function ContentBlock({ block }) {
  switch (block.type) {
    ...
    case 'read':
      return (
        <p className="draw-near-read">
          <strong>
            Read <a href={block.href}>{block.reference}</a>
            {block.suffix}
          </strong>
        </p>
      );
    ...
    case 'pause':
      return <p className="draw-near-pause"><em>{block.text}</em></p>;
    case 'closing':
...
function DrawNearPage() {
  const { hero, introduction, metadata, relatedResources, sections, translationNote } =
    DRAW_NEAR_CONTENT;
