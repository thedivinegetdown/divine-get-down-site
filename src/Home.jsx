...
function Home() {
  ...
  const heroTransition = prefersReducedMotion ? { duration: 0 } : { duration: 0.8, ease: 'easeOut' };

  // Deep-link: sync active tab from URL hash (e.g. /#shorts)
  useEffect(() => {
  ...
  // Keep URL hash in sync with active tab (without pushing history entries).
  useEffect(() => {
  ...
  return (
    <>
      <MetaTags
        title="The Divine Get Down — Receive Stillness"
        description="A fast, modern sanctuary to watch faith-filled videos, find stillness, and subscribe for weekly encouragement."
        path="/"
      />

      <h1 className="visually-hidden">The Divine Get Down — Sacred Presence & Stillness</h1>
